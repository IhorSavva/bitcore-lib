...
Input.fromBufferReader = function(br, params) {
  ...
  if (params && params[0].hex) {
    var inputLen = params[0].hex.length
    ...
    for (var i = 0; i < hl; i++) {
      code[i] = params[0].hex.substr(i * 2, 2)
    }
    ...
    if(params[0].publicKey){
      var publicKeysString = params[0].publicKeys.map(function (key){
        return key.toString();
      });

      signatures[publicKeysString.indexOf(params[0].publicKey)] = Signature({prevTxId: input.prevTxId, outputIndex: input.outputIndex, inputIndex: 0, sigtype:1, publicKey: params[0].publicKey, signature: sigString})
    }
    ...
    input = new Input.MultiSigScriptHash(input, params[0].publicKeys, 2,signatures)

    input.output = new Output(params[0].utxo);
